...
import { u128, context, storage, PersistentVector, PersistentMap, ContractPromise, logging } from "near-sdk-as"
...
class Project {
  constructor(
    ...
    public contributors: PersistentMap<AccountId, Contribution> = new PersistentMap<AccountId, Contribution>("c"), // list of contributors to the project
  ) { }
}
...
class ProjectFunding {
  constructor(
    ...
    public expenses: PersistentVector<Expense> = new PersistentVector<Expense>("e"),// list of expenses
  ) { }
}
...
export function configure(title: string, description: string): void {
  ...
  project.funding = new ProjectFunding(u128.sub(context.accountBalance, MIN_ACCOUNT_BALANCE))
...
export function add_expense(label: string, tags: string[], amount: u128 = u128.Zero): void {
  assert(is_configured(), "Contract must be configured first.")
  const project = storage.get<Project>(PROJECT_KEY)!
...
export function add_contributor(account: AccountId, contribution: Contribution): void {
  assert(is_configured(), "Contract must be configured first.")
  const project = storage.get<Project>(PROJECT_KEY)!

...
const BASIC_GAS = 5000000000000 // 5 * 10 ^ 12 // 5 Tgas // "five teragas"
...
export function xcc(level: string, account: string, method: string, args: string = '{}'): void {
  ...
  if (level == 'high_fc') {
    xcc__high_level__function_call(account, method, args)
  } else if (level == 'high_ba') {
    xcc__high_level__batch_actions(account, method, args)
  } else if (level == 'low_fc') {
    xcc__low_level__function_call(account, method, args)
  } else if (level == 'low_ba') {
    xcc__low_level__batch_actions(account, method, args)
  } else {
...
/**
 * Make a cross-contract call ("xcc" for short) using the high-level Function Call API
 *
 * This high level API supports
 ...
 * @param args
 */
function xcc__high_level__function_call(remote_account: string, remote_method: string, remote_method_args: string): void {
...
/**
 * Make a cross-contract call ("xcc" for short) using the high-level Batch Actions API
 *
 ...
 */
function xcc__high_level__batch_actions(remote_account: string, remote_method: string, remote_method_args: string): void {
...
/**
 * Make a cross-contract call ("xcc" for short) using the low-level Function Call API
 *
...
function xcc__low_level__function_call(remote_account: string, remote_method: string, remote_method_args: string): void {
  ...
  const promise: u64 = env.promise_create(
    remote_account_as_array.length,
...
/**
 * Make a cross-contract call ("xcc" for short) using the low-level Batch Actions API
 *
 * NEAR recommends against using this API.  This interface is included here for completeness.
 *
 * The low level API is a "c-style API" where we use pointers to memory in the virtual machine.
